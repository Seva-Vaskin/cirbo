...
class CircuitSatInstance:
    ...
    def assign(self, label: str, value: bool) -> AssignmentStatus:
        ...
        self.circuit = RemoveConstantGates().transform(self.circuit)
    ...
    def apply_trivial_assignments(self) -> AssignmentStatus:
        assignments: tp.List[tp.Tuple[str, bool]] = []
        for gate_label in self.circuit.gates:
            ...
            if not false_ok and not true_ok:
                ...
                assignments.append((gate_label, True))
            elif not true_ok:
                assignments.append((gate_label, False))

        for (gate_label, gate_value) in assignments:
            assignment_status = self.assign(gate_label, gate_value)
            if assignment_status != AssignmentStatus.OK:
                return assignment_status
        return AssignmentStatus.OK
